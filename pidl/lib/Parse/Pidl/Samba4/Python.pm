...
use Parse::Pidl::CUtil qw(get_value_of get_pointer_to get_array_element);
...
sub ConvertObjectFromPythonData($$$$$$;$)
{
	...
	if ($actual_ctype->{TYPE} eq "SCALAR" and $actual_ctype->{NAME} eq "string_array") {
		$self->pidl("$target = PyList_AsStringArray($mem_ctx, $cvar);");
		return;
	}
...
sub ConvertScalarToPython($$$)
{
	...
	if ($ctypename eq "string_array") {
		return "PyList_FromStringArray($cvar)";
	}
...
sub ConvertObjectToPythonLevel($$$$$$)
{
	...
	if ($l->{TYPE} eq "POINTER") {
		...
		if (is_charset_array($e, $l)) {
			...
			my $array_element_name = get_array_element($var_name, $counter);
			$self->ConvertObjectToPythonLevel($var_name, $env, $e, $nl, $array_element_name, $member_var, $fail);
